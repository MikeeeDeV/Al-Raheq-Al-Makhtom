...
import { X, UserCheck, ShieldCheck, LogIn } from 'lucide-react';
  ...
  useEffect(() => {
    if (isAuthModalOpen && googleBtnContainerRef.current) {
      renderOfficialGoogleButton(
        ...
        (err) => {
          console.warn('Google official SDK notice:', err);
        }
      );
    }
  }, [isAuthModalOpen]);
      ...
      >
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-[11px] text-m3-onSurface-variant text-center font-medium">
              أو اضغط للمتابعة المباشرة كـ قارئ معتمد
            </p>
            ...
            <motion.button
              ...
              className="w-full py-3 px-4 bg-white dark:bg-slate-800 text-slate-800 dark:text-white font-bold rounded-2xl border border-slate-300 dark:border-slate-700 shadow-xs hover:shadow-md transition flex items-center justify-center gap-3 cursor-pointer text-xs"
            >
              ...
              <span>{isLoadingGoogle ? 'جاري الاتصال...' : 'تسجيل الدخول الفوري بـ Google'}</span>
            </motion.button>
          </div>
