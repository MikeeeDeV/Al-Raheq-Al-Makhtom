  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-40 bg-white/85 dark:bg-slate-950/85 backdrop-blur-2xl border-b border-slate-200/60 dark:border-slate-800/60 transition-all shadow-xs font-arabic dir-rtl pt-[env(safe-area-inset-top,0px)]">
        ...
        <div className="max-w-7xl mx-auto px-2.5 sm:px-6 h-13 sm:h-16 flex items-center justify-between gap-1.5 sm:gap-4">
          {/* Logo & Brand Emblem */}
          <motion.div
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.96 }}
            onClick={() => setCurrentView('home')}
            className="flex items-center gap-2 sm:gap-3.5 cursor-pointer group shrink-0 select-none"
          >
            <div className="relative">
              ...
              <Sparkles className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-amber-400 absolute -top-1 -right-1 animate-pulse" />
            </div>

            <div>
              <div className="flex items-center gap-1 sm:gap-1.5">
                <h1 className="font-black text-[13px] sm:text-lg leading-tight tracking-tight bg-gradient-to-r from-emerald-800 via-teal-900 to-emerald-700 dark:from-emerald-300 dark:via-teal-200 dark:to-emerald-400 bg-clip-text text-transparent">
                  الرحيق المختوم
                </h1>
                <span className="px-1.5 py-0.2 bg-gradient-to-r from-emerald-500/10 to-teal-500/10 text-emerald-700 dark:text-emerald-300 rounded-md text-[8px] sm:text-[9px] font-bold border border-emerald-500/20">
                  تفاعلي
                </span>
              </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}
              className="flex items-center gap-1 px-2 py-1 sm:px-3 sm:py-1.5 bg-gradient-to-r from-amber-500/15 via-orange-500/15 to-amber-500/15 text-amber-900 dark:text-amber-300 rounded-full border border-amber-500/30 text-[11px] sm:text-xs font-black shrink-0 shadow-xs cursor-default"
              title="سلسلة أيام القراءة المتتالية"
            >
              <Flame className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-500 fill-amber-500 animate-bounce-gentle" />
              <span className="font-mono">{streak}d</span>
            </motion.div>

            {/* Mobile Icon Capsule Container */}
            <div className="flex items-center gap-0.5 sm:gap-1 bg-slate-100/70 dark:bg-slate-900/70 p-0.5 sm:p-1 rounded-full border border-slate-200/60 dark:border-slate-800/60 shadow-xs">
              {/* Social Share Button */}
              <motion.button
              ...
              >
                <Share2 className="w-3.5 h-3.5 sm:w-4.5 sm:h-4.5" />
              </motion.button>
              ...
              >
                <MessageSquare className="w-3.5 h-3.5 sm:w-4.5 sm:h-4.5 text-emerald-600 dark:text-emerald-400" />
              </motion.button>
