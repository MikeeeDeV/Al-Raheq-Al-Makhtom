...
export const ReaderView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Reader Control Toolbar (M3 Surface Container) */}
      <div className="sticky top-16 z-30 bg-m3-surface/95 dark:bg-m3-surface-dark/95 backdrop-blur-md p-3 md:p-4 rounded-3xl border border-m3-outline-variant/30 shadow-m3-2 space-y-3">
        {/* Top Control Line */}
        <div className="flex flex-wrap items-center justify-between gap-3">
          {/* Right: Page Stepper Navigation */}
          <div className="flex items-center gap-1.5 bg-m3-surface-dim dark:bg-m3-surface-darkContainer p-1 rounded-full border border-m3-outline-variant/20">
            {/* Next in RTL */}
            <button
              onClick={() => setCurrentPage(currentPage + (viewMode === 'double' ? 2 : 1))}
              disabled={currentPage >= totalPages}
              className="p-2 rounded-full text-m3-onSurface hover:bg-black/5 dark:hover:bg-white/5 disabled:opacity-30 disabled:pointer-events-none transition"
            ...
            {/* Jump Input */}
            <form onSubmit={handlePageSubmit} className="flex items-center gap-1 px-2 text-xs font-semibold">
              <input
                type="number"
                min={1}
                max={totalPages}
                ...
                className="w-12 h-8 text-center bg-white dark:bg-m3-surface-darkDim border border-m3-outline-variant/40 rounded-lg text-m3-onSurface focus:outline-none focus:border-m3-primary"
              />
              <span className="text-m3-onSurface-variant">/ {totalPages}</span>
            </form>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2.5 bg-m3-surface-dim dark:bg-m3-surface-darkContainer rounded-full text-m3-onSurface hover:bg-m3-primary-container/40 disabled:opacity-30 transition"
              title="تصغير"
            >
              <ZoomOut className="w-4 h-4" />
            </button>

            <span className="text-xs font-bold w-12 text-center text-m3-onSurface font-sans">
            ...
            <button
              ...
              className="p-2.5 bg-m3-surface-dim dark:bg-m3-surface-darkContainer rounded-full text-m3-onSurface hover:bg-m3-primary-container/40 transition"
          ...
          <div className="flex items-center gap-2">
            {/* Theme Picker Segmented Buttons */}
              ...
              {themeOptions.map((t) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition ${
                    readingTheme === t.id
                      ? 'bg-m3-primary-container text-m3-primary-onContainer font-bold shadow-sm'
            ...
            <button
              ...
              className="relative p-2.5 bg-m3-surface-dim dark:bg-m3-surface-darkContainer rounded-full text-m3-onSurface hover:bg-m3-primary-container/40 transition"
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-xs text-m3-onSurface-variant font-medium">
            <span>نسبة إنجاز قراءة الكتاب: {progressPercentage}%</span>
            <span>الصفحة {currentPage} من أصل {totalPages}</span>
          </div>
          <div className="w-full h-2 bg-m3-surface-dim dark:bg-m3-surface-darkDim rounded-full overflow-hidden">
            <div
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-m3-surface dark:bg-m3-surface-dark border border-m3-outline-variant/30 w-full max-w-md rounded-3xl p-6 shadow-m3-4 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-lg text-m3-onSurface">
            ...
            <form onSubmit={handleSaveBookmark} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-m3-surface-container dark:bg-m3-surface-darkContainer rounded-xl border border-m3-outline-variant/30 text-m3-onSurface text-sm focus:outline-none focus:border-m3-primary"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="اكتب أي تدوين أو فائظة استوقفتك في هذه الصفحة..."
                  value={bookmarkNote}
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setIsAddingBookmark(false)}
                  className="px-5 py-2 text-sm text-m3-onSurface-variant hover:bg-m3-surface-container rounded-full"
                >
                  إلغاء
                </button>
                <button
                  type="submit"
                  className="px-6 py-2 bg-m3-primary text-white font-medium text-sm rounded-full hover:bg-m3-primary/90 shadow-m3-1"
                >
                  حفظ الفاصل
                </button>
              </div>
      ...
      {showBookmarksDrawer && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/50 backdrop-blur-xs">
          <div className="bg-m3-surface dark:bg-m3-surface-dark w-full max-w-sm h-full p-6 shadow-m3-5 overflow-y-auto space-y-6 flex flex-col justify-between">
            <div className="space-y-6">
              ...
              {bookmarks.length === 0 ? (
                <div className="text-center py-12 text-m3-onSurface-variant space-y-2">
                  ...
                  <p className="font-semibold">لا توجد فواصل محفوطة بعد</p>
                  ...
                  {bookmarks.map((bm) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <h4 className="font-bold text-sm text-m3-onSurface">{bm.title}</h4>
                        ...
                        <button
                          ...
                          className="p-1 text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-full transition opacity-70 group-hover:opacity-100"
                          title="حذف الفاصل"
                        >
                      ...
                      <button
                        ...
                        className="w-full mt-2 py-1.5 bg-m3-primary-container text-m3-primary-onContainer rounded-full text-xs font-semibold hover:bg-m3-primary/20 transition flex items-center justify-center gap-1"
                      >
            ...
            <button
              onClick={() => setShowBookmarksDrawer(false)}
              className="w-full py-2.5 bg-m3-surface-dim text-m3-onSurface rounded-full text-sm font-medium"
            >
              إغلاق النافذة
            </button>
          </div>
